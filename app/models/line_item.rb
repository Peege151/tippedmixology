class LineItem < ActiveRecord::Base
  belongs_to :product
  belongs_to :cart
  after_create :set_order_preview_weight,  :set_order_preview_total_units, :set_package_dimentions
  after_update :set_order_preview_weight, :set_order_preview_total_units, :set_package_dimentions
  	#max capactiy here
  	def over_order_cap?
  		if self.quantity > 5
  			return true
    ...
    def set_package_dimentions
      ...
      elsif self.cart.order_preview.total_units == 1.0 #if one unit...(1 item)
      ...
      elsif self.cart.order_preview.total_units <= 3.0 #(2-3 items)
      ...
      elsif self.cart.order_preview.total_units <= 6.0 # (4-6 items)
      ...
      elsif self.cart.order_preview.total_units <= 12.0 #(7-12 items)
              self.cart.order_preview.width = 12.0
              self.cart.order_preview.height = 12.0
              self.cart.order_preview.length = 18.0
      else      
            #big-ass box (not sure, 24x24x12) (up to 32 units)
            self.cart.order_preview.width = 12.0
